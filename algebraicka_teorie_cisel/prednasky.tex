...
\textbf{Věta (Chao Ko, 1965):} Nechť $q \ge 5$ je prvočíslo. Diofantická rovnice $x^2 - y^q = 1$ má pouze triviální řešení $(\pm 1, 0)$ a $(0, -1)$ .
...
\vspace{0.5em}
\begin{proof}
Předpokládejme pro spor, že $q$ nedělí $x$ ($\neg(q|x)$). Rovnici si přepíšeme jako $x^2 = y^q + 1$ a pravou stranu algebraicky rozložíme:
$$x^2 = (y + 1) \cdot \frac{y^q + 1}{y + 1} $$

Z pomocného Lemmatu 3.1.1 plyne, že největší společný dělitel těchto dvou činitelů musí dělit číslo $q$. Protože jsme ale předpokládali, že $q$ nedělí $x$, nemůže $q$ dělit ani $x^2$, a tudíž musí být tito dva činitelé absolutně nesoudělní. 

Podle principu mocnin PP1 (Tvrzení 1.2.2) z toho plyne, že činitel $(y + 1)$ musí být čtverec, tedy $y + 1 = u^2$ pro nějaké $u \in \mathbb{N}$. (Z parity řešení víme, že $y$ je sudé, a tedy $u$ je liché ). Z rovnice si nyní můžeme zkonstruovat dvě různá řešení Pellovy rovnice $X^2 - yY^2 = 1$:
$$x^2 - y \cdot (y^{(q-1)/2})^2 = 1 \quad \text{a} \quad u^2 - y \cdot 1^2 = 1 $$

Máme tedy řešení $\langle x, y^{(q-1)/2} \rangle$ a zjevně minimální řešení $\langle u, 1 \rangle$ . Podle teorie Pellových rovnic (Tvrzení 1.1.1) musí existovat přirozené číslo $m$ takové, že v oboru $\mathbb{Z}[\sqrt{y}]$ platí:
$$x + y^{(q-1)/2}\sqrt{y} = (u + \sqrt{y})^m $$

Pokud tuto rovnost rozvineme a redukujeme modulo $y$, zjistíme, že $y$ musí dělit výraz $m u^{m-1}$ . Protože je $y$ sudé a $u$ liché, musí být index $m$ sudé číslo. 
Když pravou stranu přepíšeme pro sudé $m$ do tvaru $(u^2 + y + 2u\sqrt{y})^{m/2}$ a zredukujeme modulo $u$, zjistíme, že $u$ musí dělit $y^{(q-1)/2}$ . 
Avšak z předpokladu $y + 1 = u^2$ víme, že $u$ a $y$ jsou nesoudělná. Z toho nutně plyne $u = \pm 1$, což po dosazení dává $y = 0$ . To je ale spor s tím, že hledáme nenulové řešení. Tvrzení je tak dokázáno a $q$ musí dělit $x$.
\end{proof}
...
$$x - 1 = 2^{q-1}a^q \quad \text{a} \quad x + 1 = 2b^q $$
...
Jelikož jsou čísla $2a$ a $b^2$ nesoudělná, podle Lemmatu 3.1.1 platí, že největší společný dělitel těchto dvou činitelů na levé straně dělí $q$. Nastávají tak jen dvě možnosti pro tento největší společný dělitel:
\begin{itemize}
    \item \textbf{Dělitel je $q$:} Pak je výraz $(x-3)^2$ dělitelný $q$, z čehož rovnou plyne $x \equiv 3 \pmod q$ (respektive $x \equiv \pm 3 \pmod q$ před případnou změnou znaménka v Lemmatu).
    \item \textbf{Dělitel je 1 (činitelé jsou nesoudělní):} Podle principu mocnin PP1 musí existovat $c \in \mathbb{N}$ takové, že $b^2 - 2a = c^2$. Ze vzdálenosti nejbližších čtverců lze odvodit nerovnost $|a| \ge |b|$ . Z druhé strany, porovnáním velikostí původních předpisů pro $a^q$ a $b^q$, platí hraniční nerovnost $|a|^q < |b|^q$, z čehož vyplývá $|a| < |b|$ . To je přímý spor, takže tento případ nemůže nastat.
\end{itemize}
Tím je dokázáno, že nutně platí $x \equiv \pm 3 \pmod q$.
\end{proof}
...
Předpokládejme, že pro nějaké prvočíslo $q \ge 5$ existuje nenulové řešení $x, y \in \mathbb{Z}^*$ rovnice $x^2 - y^q = 1$. 
...
Tato podmínka říká, že prvočíslo $q$ musí dělit číslo 3. Jediné prvočíslo, které dělí 3, je však samotná 3. Náš předpoklad ale od začátku striktně požaduje, aby $q \ge 5$. 

Dostali jsme jasný matematický spor. Diophantická rovnice $x^2 - y^q = 1$ tedy pro žádné prvočíslo $q > 3$ nenulové celočíselné řešení nemá.
